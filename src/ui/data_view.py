...
from lang import _
from converters.data_conv import convert_data, get_format_from_ext, DATA_FORMATS

EXT_MAP = {"CSV": ".csv", "JSON": ".json"}


class DataConvertView:
    ...
    def _gen_out_name(self):
        ...
        ext = EXT_MAP.get(self._out_menu.get(), ".csv")
    ...
    def _load_file(self, fp):
        ...
        if fmt:
            ...
            opp = {"CSV": "JSON", "JSON": "CSV"}
            self._out_menu.set(opp.get(fmt, "JSON"))
    ...
    def _on_result(self, dst, error):
        ...
        self.app.add_history(
            {"src": os.path.basename(self._src_path), "dst": os.path.basename(dst)}, "data"
        )
