...
from lang import _
from converters.video_conv import (
    convert_video, get_format_from_ext, VIDEO_OUTPUT_FORMATS,
    OUTPUT_FORMATS as VIDEO_FMTS, EXT_MAP as VIDEO_EXT_MAP,
)

EXT_MAP = VIDEO_EXT_MAP


class VideoConvertView:
    ...
    def _build(self):
        ...
        ctk.CTkLabel(
            opt_frame, text=f"  {_('bitrate')}",
            font=ctk.CTkFont(family="Consolas", size=12),
            text_color="#666666",
        ).pack(side="left")

        self._bitrate_var = ctk.StringVar(value="2000k")
        ctk.CTkOptionMenu(
            opt_frame, values=["1000k", "2000k", "4000k", "8000k", "16000k"],
    ...
    def _gen_out_name(self):
        ...
        ext = EXT_MAP.get(self._out_menu.get(), ".mp4")
    ...
    def _browse(self):
        fp = fd.askopenfilename(
            ...
            filetypes=[("Video", "*.mp4 *.avi *.mov *.mkv *.webm *.gif")],
        )
        if fp:
            self._load_file(fp)
    ...
    def _load_file(self, fp):
        ...
        if fmt:
            ...
            opp = {"MP4": "AVI", "AVI": "MP4", "MOV": "MP4",
                   "MKV": "MP4", "WEBM": "MP4", "GIF": "MP4"}
            self._out_menu.set(opp.get(fmt, "MP4"))
    ...
    def _convert(self):
        ...
        self._status.configure(text=f"[ {_('converting')} {os.path.basename(src)} ]")

        def do():
            try:
                convert_video(src, dst, out_fmt, bitrate=bitrate)
    ...
    def _on_result(self, dst, error):
        ...
        self.app.add_history(
            {"src": os.path.basename(self._src_path), "dst": os.path.basename(dst)}, "video"
        )
