...
from lang import _
from converters.audio_conv import (
    convert_audio, get_format_from_ext, AUDIO_OUTPUT_FORMATS,
    OUTPUT_FORMATS as AUDIO_FMTS, EXT_MAP as AUDIO_EXT_MAP,
)

EXT_MAP = AUDIO_EXT_MAP


class AudioConvertView:
    ...
    def _build(self):
        ...
        self._path_entry = ctk.CTkEntry(
            card,
            placeholder_text="выберите файл...",
        ...
        ctk.CTkLabel(
            opt_frame, text="  битрейт:",
            font=ctk.CTkFont(family="Consolas", size=12),
            text_color="#666666",
        ).pack(side="left")

        self._bitrate_var = ctk.StringVar(value=self.app.config.get("audio_bitrate", "192k"))
        ctk.CTkOptionMenu(
            opt_frame, values=["128k", "192k", "256k", "320k"],
    ...
    def _gen_out_name(self):
        ...
        ext = EXT_MAP.get(self._out_menu.get(), ".mp3")
    ...
    def _load_file(self, fp):
        ...
        if fmt:
            ...
            opp = {"MP3": "WAV", "WAV": "MP3", "OGG": "MP3", "FLAC": "MP3", "M4A": "MP3"}
            self._out_menu.set(opp.get(fmt, "MP3"))
    ...
    def _convert(self):
        ...
        def do():
            try:
                convert_audio(src, dst, out_fmt, bitrate=bitrate)
    ...
    def _on_result(self, dst, error):
        ...
        self.app.add_history(
            {"src": os.path.basename(self._src_path), "dst": os.path.basename(dst)}, "audio"
        )
